...
function connect_person_work_nodes() {
    if (connect_node_parent && connect_node_modal_active) {
        ...
        if (work_or_person == "work") id = person;
        else id = work;
        new_node = {
            'id': id,
            'depth': 1,
...
function draw_tree(error, treeData) {
    ...
    if (!getParameterByName('work')) work_or_person = 'person';
    else work_or_person = 'work';
    var menu = [{
        title: "Connect Node",
        action: function(elm, d, i) {
            if (work_or_person == "person") {
                $("#ConnectWorkNode").val('');
                $("#ConnectPersonNode").val(d.id);
                $("#ConnectWorkNode").prop('disabled', false);
                $("#ConnectPersonNode").prop('disabled', true);
                $('#CreatePersonNode').focus();
            } else {
                $("#ConnectWorkNode").val(d.id);
                $("#ConnectPersonNode").val('');
                $("#ConnectWorkNode").prop('disabled', true);
                $("#ConnectPersonNode").prop('disabled', false);
